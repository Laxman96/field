...
export class CardPaymentComponent implements OnInit, OnDestroy {
  ...
  //   cardNumber: new FormControl('',  [Validators.required, Validators.email]),
  //   expirationMonth: new FormControl('',  Validators.required),
  //   expirationYear: new FormControl('',  Validators.required),
  //   cardCVVNumber: new FormControl('',  Validators.required),
  // })

  constructor(private formBuilder: FormBuilder, private facade: CreditCardPaymentFacade, private route: Router) { }
  ...
  buildForm() {
    this.paymentForm = this.formBuilder.group({
      ...
      expirationMonth: ['', [Validators.required,Validators.minLength(1),Validators.maxLength(2),Validators.max(12)]],
 ...
 onSubmit() {
  ...
  if (this.paymentForm.status === 'VALID') {
    ...
    this.data = JSON.stringify(paymentFormData);
    console.log(this.data, 'fgfg');
  } else {
